...
func TestRouterGreetWithNameContainingSpaces(t *testing.T) {
	t.Parallel()

	appService := &service.MockService{}
	appService.On("Greet", "Jane Doe").Return("Hello, Jane Doe!")

	app := fiber.New()
	NewRouter(appService).Register(app)

	request := httptest.NewRequest(http.MethodGet, "/greet?name=Jane%20Doe", nil)
	response, err := app.Test(request, fiber.TestConfig{Timeout: 0, FailOnTimeout: false})
	require.NoError(t, err)
	...
	require.Equal(t, http.StatusOK, response.StatusCode)
	require.JSONEq(t, `{"message":"Hello, Jane Doe!"}`, string(body))
	appService.AssertExpectations(t)
}
